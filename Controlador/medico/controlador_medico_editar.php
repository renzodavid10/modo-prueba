...
$idusuario = htmlspecialchars($_POST['idU'], ENT_QUOTES, 'UTF-8');
...
$idM =  htmlspecialchars($_POST['idM'], ENT_QUOTES, 'UTF-8');
...
if ($contador > 0) {
   ...
   $consulta = $MU->modificar_medico($idM,$dni, $nombre, $aPat, $aMat,$espec, $celular,$fNa, $dire
    ,$idusuario,$usuario,$email,$rol);
    echo $consulta;
    if ($consulta == 1) {
        if (!empty($nombrefoto)) {
